...
# lmmsDE2 modifications by Roy Lardenoije
...
#' @usage lmmsDE2(data, time, sampleID, group, type,
...
#' @return lmmsDE2 returns an object of class \code{lmmsde} containing the following components:
...
# @export
#setGeneric('lmmsDE2',function(data,time,sampleID,group,type,experiment,basis,knots,keepModels,numCores){standardGeneric('lmmsDE2')})
...
# @rdname lmmsDE2-methods
# @aliases lmmsDE2 lmmsDE2,matrixOrframe,numeric,factorOrcharacterOrnumeric,
...
# @exportMethod lmmsDE2

#setMethod('lmmsDE2',c(data="matrixOrframe",time="numeric",sampleID="character",group="character",type="missing",experiment="missing",basis="missing",knots="missing",keepModels="missing",numCores="missing"), function(data,time,sampleID,group,type,experiment,basis,knots,keepModels,numCores){
#  lmmsDE2Para(data=data,time=time,sampleID=sampleID,group=group,type=type,experiment=experiment,basis=basis,keepModels=keepModels,numCores=numCores)
#})


#setMethod('lmmsDE2',c(data="matrixOrframe",time="numeric",sampleID="factorOrcharacterOrnumeric",group="factorOrcharacterOrnumeric",type="missingOrcharacter",experiment="missingOrcharacter",basis="missingOrcharacter",knots="missingOrnumeric",keepModels="missingOrlogical",numCores="missingOrnumeric"), function(data,time,sampleID,group,type,experiment,basis,knots,keepModels,numCores){
#  lmmsDE2Para(data=data,time=time,sampleID=sampleID,group=group,type=type,experiment=experiment,basis=basis,keepModels=keepModels,numCores=numCores)
#})
# @method lmmsDE2 data.frame
#' @docType methods
#' @rdname lmmsDE2-methods
#' @export
lmmsDE2 <- function (data, time, sampleID, group, type, experiment, basis, knots, keepModels, numCores){
    ...
    if (type == "time*group") 
        type <- "grouptime"
    experiment.collection <- c("timecourse", "longitudinal1", 
                               "longitudinal2", "all")
    if (!experiment %in% experiment.collection) 
        stop(paste("Chosen experiment is not available. Choose ", 
                   paste(experiment.collection, collapse = ", ")))
    basis.collection <- c("cubic", "p-spline", "cubic p-spline")
    if (!basis %in% basis.collection) 
        stop(paste("Chosen basis is not available. Choose ", 
                   paste(basis.collection, collapse = ", ")))
    type.collection <- c("all", "time", "group", "grouptime")
    if (!type %in% type.collection) 
        stop(paste("Chosen type is not available. Choose ", paste(type.collection, 
                                                                  collapse = ", ")))
    if (diff(range(c(length(sampleID), length(time), length(group), 
                     nrow(data)))) > 0) 
    ...
    if (missing(numCores)) {
        ...
        if (num.Cores < numCores) {
            warning(paste("The number of cores is bigger than the number of detected cores. Using the number of detected cores", 
                          num.Cores, "instead."))
        }
        else {
            num.Cores <- numCores
    ...
    tstatsdf <- matrix(NA, ncol = 3, nrow = nMolecules)
    colnames(pvalsdf) <- c("Time", "Group", "Group_Time")
    colnames(tstatsdf) <- c("Time", "Group", "Group_Time")
    fitsdf <- matrix(NA, ncol = 3, nrow = nMolecules)
    colnames(fitsdf) <- c("Time", "Group", "Group_Time")
    clusterExport(cl, list("lme", "sampleID", "time", "group", 
                           "data", "gsub", "try", "outer", "pdDiag", "melt", "dcast", 
                           "knots", "basis", "smspline", "drop.levels", "anova", 
                           "type", "pvals", "tstats", "keepModels", "lmeControl", "pdIdent", 
                           "other.reshape.group"), envir = environment())
    new.data <- parLapply(cl, 1:nMolecules, fun = function(i) {
        if (type != "all") 
            p2 <- p1 <- p3 <- t1 <- t2 <- t3 <- fitTime <- fitG <- fitTG <- NA
        expr <- data[, i]
        dataM <- other.reshape.group(Rep = sampleID, Time = time, 
                                     Data = unlist(expr), Group = group)
        ...
        if (basis %in% c("p-spline", "cubic p-spline")) {
            if (is.null(knots)) {
                K <- max(5, min(floor(length(unique(dataM$time))/4), 
                                40))
                knots <- quantile(na.omit(unique(dataM$time)), 
                                  seq(0, 1, length = K + 2))[-c(1, K + 2)]
            }
        ...
        if (type == "group" | type == "all") {
            ...
            if (experiment == "timecourse" | experiment == "all") {
                fit0T <- NULL
                fit0T <- try(lme(Expr ~ 1, data = tmp.data, random = list(all = pdIdent(~Zt - 
                                                                                            1)), na.action = na.exclude, control = lmeControl(opt = "optim"), 
                                 method = "ML"))
                fit2T <- NULL
                fit2T <- try(lme(Expr ~ as.factor(Group), data = tmp.data, 
                                 random = list(Group = pdIdent(~Zt - 1)), na.action = na.exclude, 
            ...
            if (experiment == "longitudinal1" | experiment == 
                "all") {
                fit0L1 <- NULL
            ...
            if (experiment == "all") {
                if (class(fit2T) != "try-error" & class(fit2L1) != 
                    "try-error" & !is.null(fit2T) & !is.null(fit2L1)) {
            ...
            if (experiment == "longitudinal2" | nextF) {
                ...
                if (nextF) {
                    if (class(fit2L2) != "try-error" & class(fit2L1) != 
                        "try-error" & !is.null(fit2L2) & !is.null(fit2L1)) {
            ...
            if (class(fit0) != "try-error" & class(fit2) != "try-error" & 
                !is.null(fit2) & !is.null(fit0)) {
                p1 <- anova(fit0, fit2)$`p-value`[2][1]
                t1 <- anova(fit0, fit2)$L.Ratio[2][1]
            }
            if (keepModels) {
                model.group <- fit2
            }
            ...
            if (class(fit2) != "try-error") {
                tempdf <- data.frame(tmp.data$time, tmp.data$Group, 
                                     fitted(fit2, level = 1))
                tempdf <- tempdf[!duplicated(tempdf) & !is.na(tempdf[, 
                                                                     3]), ]
                pred.group[paste(tmp.group, tmp.time) %in% paste(tempdf[, 
                                                                        2], tempdf[, 1])] <- tempdf[, 3]
            }
        }
        if (type == "time" | type == "all") {
            ...
            nextF <- F
            if (experiment == "timecourse" | experiment == "all") {
                fit0T <- try(lme(Expr ~ 1, data = tmp.data, random = list(all = pdIdent(~1)), 
                                 na.action = na.exclude, control = lmeControl(opt = "optim"), 
                                 method = "ML"))
                fit3T <- try(lme(Expr ~ time, data = tmp.data, 
                                 random = list(all = pdIdent(~Zt - 1)), na.action = na.exclude, 
                                 control = lmeControl(opt = "optim"), method = "ML"))
                if (experiment == "timecourse") {
                    fit0 <- fit0T
                    fit3 <- fit3T
            ...
            if (experiment == "all") {
                if (class(fit3L1) != "try-error" & class(fit3T) != 
                    "try-error" & !is.null(fit3L1) & !is.null(fit3T)) {
            ...
            if (experiment == "longitudinal2" | nextF) {
                ...
                if (nextF) {
                    if (class(fit3L1) != "try-error" & class(fit3L2) != 
                        "try-error" & !is.null(fit3L1) & !is.null(fit3L2)) {
            ...
            if (class(fit0) != "try-error" & class(fit3) != "try-error" & 
                !is.null(fit0) & !is.null(fit3)) {
                p2 <- anova(fit0, fit3)$`p-value`[2][1]
                t2 <- anova(fit0, fit3)$L.Ratio[2][1]
            }
            if (keepModels) {
                model.time <- fit3
            }
        ...
        if (type == "grouptime" | type == "all") {
            ...
            p3TL1 <- 1
            if (experiment == "timecourse" | experiment == "all") {
                fit0T <- try(lme(Expr ~ as.factor(Group) + time, 
                                 data = tmp.data, random = list(all = pdIdent(~Zt - 
                                                                                  1)), na.action = na.exclude, control = lmeControl(opt = "optim"), 
                                 method = "ML"))
                fit5T <- try(lme(Expr ~ as.factor(Group) * time, 
                                 data = tmp.data, random = list(Group = pdIdent(~Zt - 
                                                                                    1)), na.action = na.exclude, control = lmeControl(opt = "optim"), 
                                 method = "ML"))
                if (experiment == "timecourse") {
                    fit0 <- fit0T
            ...
            if (experiment == "longitudinal1" | experiment == 
                "all") {
                fit0L1 <- try(lme(Expr ~ as.factor(Group) + time, 
                                  data = tmp.data, random = list(all = pdIdent(~Zt - 
                                                                                   1), Rep = pdIdent(~1)), na.action = na.exclude, 
                                  control = lmeControl(opt = "optim"), method = "ML"))
                fit5L1 <- try(lme(Expr ~ as.factor(Group) * time, 
                                  data = tmp.data, random = list(Group = pdIdent(~Zt - 
                                                                                     1), Rep = pdIdent(~1)), na.action = na.exclude, 
                                  control = lmeControl(opt = "optim"), method = "ML"))
                if (experiment == "longitudinal1") {
            ...
            if (experiment == "all") {
                if (class(fit5L1) != "try-error" & class(fit5T) != 
                    "try-error" & !is.null(fit5L1) & !is.null(fit5T)) {
            ...
            if (experiment == "longitudinal2" | nextF) {
                fit0L2 <- try(lme(Expr ~ as.factor(Group) + time, 
                                  data = tmp.data, random = list(all = pdIdent(~Zt - 
                                                                                   1), Rep = pdDiag(~time)), na.action = na.exclude, 
                                  control = lmeControl(opt = "optim"), method = "ML"))
                fit5L2 <- try(lme(Expr ~ as.factor(Group) * time, 
                                  data = tmp.data, random = list(Group = pdIdent(~Zt - 
                                                                                     1), Rep = pdDiag(~time)), na.action = na.exclude, 
                                  control = lmeControl(opt = "optim"), method = "ML"))
                if (experiment == "longitudinal2") {
                ...
                if (nextF) {
                    if (class(fit5L1) != "try-error" & class(fit5L2) != 
                        "try-error" & !is.null(fit5L1) & !is.null(fit5L2)) {
            ...
            if (class(fit0) != "try-error" & class(fit5) != "try-error" & 
                !is.null(fit0) & !is.null(fit5)) {
                p3 <- anova(fit0, fit5)$`p-value`[2][1]
                t3 <- anova(fit0, fit5)$L.Ratio[2][1]
            }
            if (keepModels) {
                model.time.group <- fit5
            }
            tmp.time <- sort(na.omit(unique(tmp.data$time)))
            tmp.group <- rep(unique(na.omit(tmp.data$Group)), 
                             length(tmp.time))
            tmp.time <- rep(tmp.time, each = 2)
            ...
            if (class(fit5) != "try-error") {
                ...
                tempdf <- tempdf[!duplicated(tempdf) & !is.na(tempdf[, 
                                                                     3]), ]
                pred.time.group[paste(tmp.group, tmp.time) %in% 
                                    paste(tempdf[, 2], tempdf[, 1])] <- tempdf[, 
                                                                               3]
            }
        }
        pvals <- c(p2, p1, p3)
        tstats <- c(t2, t1, t3)
        if (keepModels) {
            return(list(pvals = pvals, tstats = tstats, model.time = model.time, 
                        model.group = model.group, model.time.group = model.time.group, 
                        knots = knots, pred.time = pred.time, pred.time.group = pred.time.group, 
                        pred.group = pred.group, fits = c(fitTime, fitG, 
                                                          fitTG)))
        }
        else {
            return(list(pvals = pvals, tstats = tstats, knots = knots, pred.time = pred.time, 
                        pred.time.group = pred.time.group, pred.group = pred.group, 
                        fits = c(fitTime, fitG, fitTG)))
        }
    })
    stopCluster(cl)
    new.df.p <- matrix(sapply(new.data, "[[", "pvals"), nrow = nMolecules,
                       ncol = 3, byrow = T)
    new.df.t <- matrix(sapply(new.data, "[[", "tstats"), nrow = nMolecules,
                       ncol = 3, byrow = T)
    knots <- unique(as.vector((sapply(new.data, "[[", "knots"))))
    fits <- matrix(sapply(new.data, "[[", "fits"), nrow = nMolecules, 
                   ncol = 3, byrow = T)
    model.time <- list()
    model.time <- list()
    model.group <- list()
    ...
    pred.time <- matrix(sapply(new.data, "[[", "pred.time"), 
                        nrow = nMolecules, ncol = length(unique(time)), byrow = T)
    nameTime <- na.omit(sort(unique(time)))
    if (ncol(pred.time) == length(nameTime)) 
        colnames(pred.time) <- nameTime
    pred.group <- matrix(sapply(new.data, "[[", "pred.group"), 
                         nrow = nMolecules, ncol = (length(unique(time)) * 2), 
                         byrow = T)
    pred.group.time <- matrix(sapply(new.data, "[[", "pred.time.group"), 
                              nrow = nMolecules, ncol = (length(unique(time)) * 2), 
                              byrow = T)
    nameGroup <- paste(na.omit(sort(unique(group))), rep(na.omit(sort(unique(time))), 
                                                         each = 2))
    if (ncol(pred.group) == length(nameGroup) & ncol(pred.group.time) == 
        length(nameGroup)) 
    ...
    if (type == "all") {
        df <- data.frame(Molecule = molnames,
                         Time.t = new.df.t[, 1],
                         Time.p = new.df.p[, 1], 
                         Time.adj.p = signif(p.adjust(new.df.p[, 1], method = "BH"), 2), 
                         Group.t = new.df.t[, 2],
                         Group.p = new.df.p[, 2], 
                         Group.adj.p = signif(p.adjust(new.df.p[, 2], method = "BH"), 2), 
                         Group_Time.t = new.df.t[, 3], 
                         Group_Time.p = new.df.p[, 3], 
                         Group_Time.adj.p = signif(p.adjust(new.df.p[, 3], method = "BH"), 2))
    }
    if (type == "time") {
        df <- data.frame(Molecule = molnames, 
                         Time.t = new.df.t[, 1],
    ...
    l <- new("lmmsde", DE = df, modelsUsed = fits, modelTime = model.time, 
             modelGroup = model.group, modelTimeGroup = model.time.group, 
             knots = knots, basis = basis, type = type, experiment = experiment, 
             predTime = pred.time, predGroup = pred.group, predTimeGroup = pred.group.time)
    return(l)
}
